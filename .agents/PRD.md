...
1. **상단 네비게이션 바 (Header)**
   ...
   - (비회원) 로그인, 비회원 주문 조회
   - (회원) 마이페이지
   - (반응형) 모바일 환경(md 미만)에서는 우측 상단 햄버거 메뉴 아이콘 제공. 클릭 시 위에서 아래로(Top) 내려오는 풀 너비(Full-width) Drawer를 화면 전체를 차지하도록 구현하여 메뉴를 노출합니다. 모바일 Drawer 내부의 로그인 버튼은 **연한 주황색(bg-orange-400) 배경에 모서리가 완전히 둥근 알약 형태(rounded-full)의 박스 버튼**으로 시각적 포인트를 부여하고, 둥근 모서리가 잘리지 않도록 좌우 여백을 적용합니다.
2. **Body**
   - 각 페이지 별 주요 내용 렌더링
3. **Footer**
