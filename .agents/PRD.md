...
| :---: | :--- | :--- | :--- | :--- | :--- | :---: | :--- |
| 1 | **인증 (Auth)** | 소셜 로그인 및 유저 DB 자동 생성 | 구글/카카오 로그인 성공 | 1. 소셜 로그인 완료 후 콜백 라우트(`/api/auth/callback`) 호출<br>2. 신규 사용자 정보가 `auth.users`에 적재될 때 DB 트리거 동작 확인 | 1. `public.users`에 해당 유저 행이 닉네임, 이메일과 함께 자동 복사되어 적재됨<br>2. 재로그인 시 기존의 커스텀 닉네임을 덮어쓰지 않음 | [x] | DB 트리거 및 닉네임 보존 로직 검증 완료 |
| 2 | **인증 (Auth)** | 세션 기반 로그아웃 | 회원 로그인 상태 | 1. `/api/auth/logout` API에 POST 요청 전송<br>2. 쿠키 세션 및 Supabase 인증 토큰 파기 확인 | 1. 성공 응답(`200 OK`) 반환 및 브라우저 세션 쿠키 소멸<br>2. 이후 로그인 전용 페이지(/my-page) 접근 시 `/auth`로 튕김 | [x] | API 및 마이페이지 연동 완료 |
| 3 | **인증 (Auth)** | 비회원 게스트 임시 로그인 | 비회원 정보 존재 | 1. `/api/auth/guest` API로 존재하지 않는 번호/틀린 비밀번호 전송<br>2. 올바른 정보 전송 시 암호화 세션 쿠키 수립 여부 검증 | 1. 잘못된 정보 전달 시 `401 Unauthorized` 에러 반환<br>2. 성공 시 임시 식별 토큰 발급 및 주문 조회 권한 획득 | [ ] | 백엔드 API 구현 예정 |
| 4 | **사용자 (Users)** | 내 정보 조회 및 닉네임 변경 | 로그인 상태 | 1. `/api/users/me` API로 공백이나 특수문자로 채워진 잘못된 닉네임 PATCH 전송<br>2. 정상 닉네임 PATCH 전송 후 F5 새로고침 수행 | 1. 유효하지 않은 닉네임 요청 시 `400 Bad Request` 처리<br>2. 성공 시 `public.users` 테이블에 실시간 반영 및 재로딩 시 유지 | [x] | API 및 마이페이지 연동 완료 |
| 5 | **주문 (Orders)** | 주문 생성 및 금액 무결성 검증 | 로그인 상태 | 1. `/api/orders` API로 가짜 단가(예: 단판을 100원으로 변조) 요청 전송<br>2. 다회권 보유 회원이 이용권 차감 주문 전송 시 처리 | 1. 변조된 금액 전달 시 `400 Bad Request` 에러 반환 (서버 가격표 비교 검증)<br>2. 이용권 차감 시 결제 모듈 호출 없이 즉시 주문 접수 완료 | [ ] | 백엔드 API 구현 예정 |
| 6 | **결제 (Payments)** | 결제 승인 API 및 웹훅 연동 | 주문 접수 완료 상태 | 1. Toss Payments 결제 완료 후 `/api/payments/confirm` 호출<br>2. 외부 Toss Webhook을 통해 실시간 결제 완료 이벤트 수신 | 1. 토스 승인 API 통신 후 DB `payments` 기록 생성 및 주문 `payment_status`가 `paid`로 업데이트됨 | [ ] | 토스페이먼츠 승인 연동 시 진행 예정 |
| 7 | **해몽 (Dream Results)** | 결과 조회 및 공개 여부 토글 | 로그인 상태 | 1. 소유권이 없는 타인이 `/api/dream-result/[id]` 결과 조회를 시도<br>2. 결과 페이지에서 `is_public` 노출 여부를 광클 토글 | 1. 타인 결과 접근 시 `403 Forbidden` 차단 처리<br>2. 토글 API 호출 시 `public.dream_results` 테이블의 `is_public`이 정상적으로 반전 및 동기화됨 | [x] | 소유권 RLS 정책 및 API 연동 완료 |
| 8 | **이용권 (Passes)** | 이용권 충전/차감 트랜잭션 | 다회권 결제 완료 / 차감 발생 | 1. 다회권(5회/10회) 결제 완료 API 트리거<br>2. `pass_transactions` 테이블에 충전 및 차감 이력이 원자적으로 적재되는지 검증 | 1. 트랜잭션 적재 성공과 동시에 `users.remaining_interprets` 갯수가 양수/음수만큼 정확하게 합산 반영됨 | [ ] | 백엔드 API 구현 예정 |
| 9 | **관리자 (Admin)** | 관리자 전용 API 호출 권한 제어 | 일반 회원 또는 비회원 로그인 상태 | 1. 일반 계정으로 `/api/admin/orders` 또는 `/api/admin/users` API 직접 호출 시도 | 1. 관리자 권한(`admin` role) 검증 단계에서 실패하여 `403 Forbidden` 반환 및 차단 | [ ] | 관리자 API 권한 검증 단계 구현 예정 |
