...
import PhoneRegistrationModal from "@/components/auth/phone-registration-modal";
...
const Header = () => {
  ...
  useEffect(() => {
    const checkUserRole = async (currentUser: any) => {
      ...
      try {
        ...
        // 회원이면서 연락처가 없는 경우 등록 모달 표시
        if (profile && profile.role !== "guest" && !profile.phone_number) {
  ...
  return (
    <>
      ...
      {/* 연락처 등록 모달 (회원가입 후 최초 1회) */}
      {showPhoneModal && (
        <PhoneRegistrationModal onSuccess={() => setShowPhoneModal(false)} />
      )}
    </>
  );
};
